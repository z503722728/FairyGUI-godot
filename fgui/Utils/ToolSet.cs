...
namespace FairyGUI.Utils
{
    ...
    public static class ToolSet
    {
        ...
        /// <summary>
        /// 90度扇形填充：从 origin 角出发扫过 90 度。
        /// amount 0→1 对应扫过角度 0→90°。
        /// </summary>
        public static int MeshAddRadial90(SurfaceTool surfaceTool, Rect vertRect, Rect uvRect, Origin90 origin, float amount, bool clockwise, int startIndex)
        {
            if (amount <= 0)
                return 0;
            if (amount >= 1)
            {
                MeshAddRect(surfaceTool, vertRect, uvRect, startIndex);
                return 4;
            }

            // --- 根据 origin 确定四个角的坐标 ---
            // oc = origin corner (扇形枢纽)
            // ea = 沿第一条边到达的角
            // eb = 沿第二条边到达的角
            // ed = 对角
            // 顺时针时先扫 oc→ea 边，再向 oc→eb 边移动
            float ocx, ocy, ocu, ocv;
            float eax, eay, eau, eav;
            float ebx, eby, ebu, ebv;
            float edx, edy, edu, edv;

            switch (origin)
            {
                case Origin90.TopLeft:
                    ocx = vertRect.xMin; ocy = vertRect.yMin; ocu = uvRect.xMin; ocv = uvRect.yMin;
                    if (clockwise)
                    {
                        eax = vertRect.xMax; eay = vertRect.yMin; eau = uvRect.xMax; eav = uvRect.yMin; // top-right
                        ebx = vertRect.xMin; eby = vertRect.yMax; ebu = uvRect.xMin; ebv = uvRect.yMax; // bottom-left
                    }
                    else
                    {
                        eax = vertRect.xMin; eay = vertRect.yMax; eau = uvRect.xMin; eav = uvRect.yMax; // bottom-left
                        ebx = vertRect.xMax; eby = vertRect.yMin; ebu = uvRect.xMax; ebv = uvRect.yMin; // top-right
                    }
                    edx = vertRect.xMax; edy = vertRect.yMax; edu = uvRect.xMax; edv = uvRect.yMax;
                    break;

                case Origin90.TopRight:
                    ocx = vertRect.xMax; ocy = vertRect.yMin; ocu = uvRect.xMax; ocv = uvRect.yMin;
                    if (clockwise)
                    {
                        eax = vertRect.xMax; eay = vertRect.yMax; eau = uvRect.xMax; eav = uvRect.yMax; // bottom-right
                        ebx = vertRect.xMin; eby = vertRect.yMin; ebu = uvRect.xMin; ebv = uvRect.yMin; // top-left
                    }
                    else
                    {
                        eax = vertRect.xMin; eay = vertRect.yMin; eau = uvRect.xMin; eav = uvRect.yMin; // top-left
                        ebx = vertRect.xMax; eby = vertRect.yMax; ebu = uvRect.xMax; ebv = uvRect.yMax; // bottom-right
                    }
                    edx = vertRect.xMin; edy = vertRect.yMax; edu = uvRect.xMin; edv = uvRect.yMax;
                    break;

                case Origin90.BottomLeft:
                    ocx = vertRect.xMin; ocy = vertRect.yMax; ocu = uvRect.xMin; ocv = uvRect.yMax;
                    if (clockwise)
                    {
                        eax = vertRect.xMin; eay = vertRect.yMin; eau = uvRect.xMin; eav = uvRect.yMin; // top-left
                        ebx = vertRect.xMax; eby = vertRect.yMax; ebu = uvRect.xMax; ebv = uvRect.yMax; // bottom-right
                    ...
                    edx = vertRect.xMax; edy = vertRect.yMin; edu = uvRect.xMax; edv = uvRect.yMin;
                    break;

                default: // BottomRight
                    ocx = vertRect.xMax; ocy = vertRect.yMax; ocu = uvRect.xMax; ocv = uvRect.yMax;
                    if (clockwise)
                    {
                        eax = vertRect.xMin; eay = vertRect.yMax; eau = uvRect.xMin; eav = uvRect.yMax; // bottom-left
                        ebx = vertRect.xMax; eby = vertRect.yMin; ebu = uvRect.xMax; ebv = uvRect.yMin; // top-right
                    ...
                    edx = vertRect.xMin; edy = vertRect.yMin; edu = uvRect.xMin; edv = uvRect.yMin;
                    break;
            }

            // --- 计算扫到的点 ---
            // 角度从 0 到 90°, 在 amount 处 tan = tan(amount * π/2)
            float angle = Mathf.Pi / 2 * amount;
            float tan = Mathf.Tan(angle);
            bool thresold = tan > 1; // amount > 0.5，扫过了45°
            ...
            if (!thresold)
            {
                ...
                // 交点在 ea→ed 边上 (比例 = tan)
                float ratio = tan;
                float px = Mathf.Lerp(eax, edx, ratio);
                float py = Mathf.Lerp(eay, edy, ratio);
                float pu = Mathf.Lerp(eau, edu, ratio);
                float pv = Mathf.Lerp(eav, edv, ratio);

                // 三角形: oc, ea, p
                surfaceTool.SetUV(new Vector2(ocu, ocv));
                surfaceTool.AddVertex(new Vector3(ocx, ocy, 0));

                surfaceTool.SetUV(new Vector2(eau, eav));
                surfaceTool.AddVertex(new Vector3(eax, eay, 0));

                surfaceTool.SetUV(new Vector2(pu, pv));
                surfaceTool.AddVertex(new Vector3(px, py, 0));
            ...
            {
                // 扇形超过了45°，交点在 ed→eb 边上
                float ratio = 1.0f / tan; // 这里用 1/tan 来映射到另一条边
                float px = Mathf.Lerp(ebx, edx, ratio);
                float py = Mathf.Lerp(eby, edy, ratio);
                float pu = Mathf.Lerp(ebu, edu, ratio);
                float pv = Mathf.Lerp(ebv, edv, ratio);
                ...
                surfaceTool.SetUV(new Vector2(edu, edv));
                surfaceTool.AddVertex(new Vector3(edx, edy, 0));
        ...
        /// 180度扇形填充：拆成两个90度
        /// </summary>
        public static int MeshAddRadial180(SurfaceTool surfaceTool, Rect vertRect, Rect uvRect, Origin180 origin, float amount, bool clockwise, int startIndex)
        {
            if (amount <= 0)
                return 0;
            if (amount >= 1)
            ...
            // Split into two 90-degree fills
            Rect[] rects = new Rect[2];
            Rect[] uvs = new Rect[2];
            Origin90[] origins = new Origin90[2];

            switch (origin)
            {
                case Origin180.Top:
                    rects[0] = new Rect(vertRect.xMin, vertRect.yMin, vertRect.width / 2, vertRect.height);
                    rects[1] = new Rect(vertRect.xMin + vertRect.width / 2, vertRect.yMin, vertRect.width / 2, vertRect.height);
                    uvs[0] = new Rect(uvRect.xMin, uvRect.yMin, uvRect.width / 2, uvRect.height);
                    uvs[1] = new Rect(uvRect.xMin + uvRect.width / 2, uvRect.yMin, uvRect.width / 2, uvRect.height);
                    if (clockwise) { origins[0] = Origin90.TopRight; origins[1] = Origin90.TopLeft; }
                    else { origins[0] = Origin90.TopLeft; origins[1] = Origin90.TopRight; }
                    break;
                case Origin180.Bottom:
                    ...
                    uvs[0] = new Rect(uvRect.xMin, uvRect.yMin, uvRect.width / 2, uvRect.height);
                    uvs[1] = new Rect(uvRect.xMin + uvRect.width / 2, uvRect.yMin, uvRect.width / 2, uvRect.height);
                    if (clockwise) { origins[0] = Origin90.BottomLeft; origins[1] = Origin90.BottomRight; }
                    else { origins[0] = Origin90.BottomRight; origins[1] = Origin90.BottomLeft; }
                    break;
                case Origin180.Left:
                    rects[0] = new Rect(vertRect.xMin, vertRect.yMin, vertRect.width, vertRect.height / 2);
                    rects[1] = new Rect(vertRect.xMin, vertRect.yMin + vertRect.height / 2, vertRect.width, vertRect.height / 2);
                    uvs[0] = new Rect(uvRect.xMin, uvRect.yMin, uvRect.width, uvRect.height / 2);
                    uvs[1] = new Rect(uvRect.xMin, uvRect.yMin + uvRect.height / 2, uvRect.width, uvRect.height / 2);
                    if (clockwise) { origins[0] = Origin90.TopLeft; origins[1] = Origin90.BottomLeft; }
                    else { origins[0] = Origin90.BottomLeft; origins[1] = Origin90.TopLeft; }
                    break;
                default: // Right
                    rects[0] = new Rect(vertRect.xMin, vertRect.yMin, vertRect.width, vertRect.height / 2);
                    rects[1] = new Rect(vertRect.xMin, vertRect.yMin + vertRect.height / 2, vertRect.width, vertRect.height / 2);
                    uvs[0] = new Rect(uvRect.xMin, uvRect.yMin, uvRect.width, uvRect.height / 2);
                    uvs[1] = new Rect(uvRect.xMin, uvRect.yMin + uvRect.height / 2, uvRect.width, uvRect.height / 2);
                    if (clockwise) { origins[0] = Origin90.BottomRight; origins[1] = Origin90.TopRight; }
                    else { origins[0] = Origin90.TopRight; origins[1] = Origin90.BottomRight; }
                    break;
            }

            int totalVerts = 0;
            float am1 = amount <= 0.5f ? amount * 2 : 1f;
            float am2 = amount <= 0.5f ? 0f : (amount - 0.5f) * 2;

            // First half
            if (am1 > 0)
            {
                int v = MeshAddRadial90(surfaceTool, rects[0], uvs[0], origins[0], am1, clockwise, startIndex + totalVerts);
                totalVerts += v;
            }
            // Second half: draw full rect first, then fill
            if (am2 > 0)
            {
                int v = MeshAddRadial90(surfaceTool, rects[1], uvs[1], origins[1], am2, clockwise, startIndex + totalVerts);
                totalVerts += v;
            }
        ...
        /// 360度扇形填充：拆成四个90度
        /// </summary>
        public static int MeshAddRadial360(SurfaceTool surfaceTool, Rect vertRect, Rect uvRect, Origin360 origin, float amount, bool clockwise, int startIndex)
        {
            if (amount <= 0)
                return 0;
            if (amount >= 1)
            ...
            // Split into four 90-degree quadrants
            Rect[] rects = new Rect[4];
            Rect[] uvs = new Rect[4];
            Origin90[] origins = new Origin90[4];

            float hw = vertRect.width / 2;
            float hh = vertRect.height / 2;
            float uhw = uvRect.width / 2;
            float uhh = uvRect.height / 2;

            // Define 4 quadrants: TL, TR, BL, BR
            rects[0] = new Rect(vertRect.xMin, vertRect.yMin, hw, hh);       // TL
            rects[1] = new Rect(vertRect.xMin + hw, vertRect.yMin, hw, hh);   // TR
            rects[2] = new Rect(vertRect.xMin + hw, vertRect.yMin + hh, hw, hh); // BR
            rects[3] = new Rect(vertRect.xMin, vertRect.yMin + hh, hw, hh);   // BL

            uvs[0] = new Rect(uvRect.xMin, uvRect.yMin, uhw, uhh);
            uvs[1] = new Rect(uvRect.xMin + uhw, uvRect.yMin, uhw, uhh);
            uvs[2] = new Rect(uvRect.xMin + uhw, uvRect.yMin + uhh, uhw, uhh);
            uvs[3] = new Rect(uvRect.xMin, uvRect.yMin + uhh, uhw, uhh);

            // The order depends on origin and direction
            int[] order;
            switch (origin)
            {
                case Origin360.Top:
                    if (clockwise) order = new[] { 1, 2, 3, 0 };
                    else order = new[] { 0, 3, 2, 1 };
                    break;
                case Origin360.Bottom:
                    if (clockwise) order = new[] { 3, 0, 1, 2 };
                    else order = new[] { 2, 1, 0, 3 };
                    break;
                case Origin360.Left:
                    if (clockwise) order = new[] { 0, 1, 2, 3 };
                    else order = new[] { 3, 2, 1, 0 };
                    break;
                default: // Right
                    if (clockwise) order = new[] { 2, 3, 0, 1 };
                    else order = new[] { 1, 0, 3, 2 };
                    break;
            }

            // Map quadrant index to Origin90
            Origin90[] quadOrigins = new Origin90[]
            {
                Origin90.BottomRight, // TL quadrant: fill from BR corner
                Origin90.BottomLeft,  // TR quadrant: fill from BL corner
                Origin90.TopLeft,     // BR quadrant: fill from TL corner
                Origin90.TopRight     // BL quadrant: fill from TR corner
            };
            ...
            {
                float localAmt;
                if (i == 0) localAmt = Mathf.Clamp(amount * 4, 0, 1);
                else if (i == 1) localAmt = Mathf.Clamp(amount * 4 - 1, 0, 1);
                else if (i == 2) localAmt = Mathf.Clamp(amount * 4 - 2, 0, 1);
                else localAmt = Mathf.Clamp(amount * 4 - 3, 0, 1);

                if (localAmt <= 0) continue;

                int qi = order[i];
                int v = MeshAddRadial90(surfaceTool, rects[qi], uvs[qi], quadOrigins[qi], localAmt, clockwise, startIndex + totalVerts);
                totalVerts += v;
            }
